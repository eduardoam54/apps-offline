 ...
 *
 * A marca e um orcamento: retangulo arredondado branco, lombada mais escura na
 * esquerda e tres linhas pautadas — mesmo desenho do app fiado, na cor de
 * destaque do orcamento (DESTAQUE.orcamento em packages/ui/src/tema.ts). Tres,
 * e nao cinco, porque abaixo de 48dp linhas finas demais viram uma mancha cinza.
 */
const { deflateSync } = require('node:zlib');
const { writeFileSync, mkdirSync } = require('node:fs');
const { join } = require('node:path');

const AZUL = [0x1d, 0x4e, 0xd8];
const AZUL_ESCURO = [0x1e, 0x3a, 0x8a];
...
/**
 * Desenha o orcamento ocupando `proporcao` do lado da imagem.
 *
 * `corLinhas` sai branco no icone monocromatico, onde o sistema aplica a
 * propria cor e qualquer detalhe colorido some.
 */
function documento(px, lado, proporcao, corCapa, corLombada, corLinhas) {
...
const principal = tela(LADO, AZUL);
documento(principal, LADO, 0.52, BRANCO, AZUL_ESCURO, AZUL);
...
gravar('android-icon-background.png', tela(LADO, AZUL));

const frente = tela(LADO, null);
documento(frente, LADO, 0.42, BRANCO, AZUL_ESCURO, AZUL);
...
documento(mono, LADO, 0.42, BRANCO, BRANCO, BRANCO);
gravar('android-icon-monochrome.png', mono);

// Splash: a marca sozinha sobre o azul definido no app.json.
const splash = tela(LADO, null);
documento(splash, LADO, 0.62, BRANCO, AZUL_ESCURO, AZUL);
