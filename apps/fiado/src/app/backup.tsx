...
export default function TelaBackup() {
  ...
  async function importar() {
    ...
    try {
      ...
      Alert.alert(
        'Substituir a caderneta?',
        `O arquivo tem ${resumo.clientes} ${resumo.clientes === 1 ? 'cliente' : 'clientes'}, ` +
          `${resumo.lancamentos} ${resumo.lancamentos === 1 ? 'lançamento' : 'lançamentos'} e ` +
          `${resumo.pagamentos} ${resumo.pagamentos === 1 ? 'pagamento' : 'pagamentos'}.\n\n` +
          ...
          { text: 'Cancelar', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                await aplicar();
                recarregar();
                Alert.alert('Pronto', 'A caderneta foi restaurada.');
              } catch (falha) {
    ...
    <ScrollView contentContainerStyle={{ padding: tema.espaco.lg, gap: tema.espaco.xl }}>
      <View style={{ gap: tema.espaco.md }}>
        ...
        <Text style={{ fontSize: tema.fonte.corpo, color: tema.cores.textoSecundario, lineHeight: 24 }}>
          Gera um arquivo com a caderneta inteira. Mande para você mesmo no WhatsApp
